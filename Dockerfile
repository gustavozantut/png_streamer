...
CMD ["/bin/bash", "-c", "gst-launch-1.0 -e multifilesrc location=\"/runs/detect/old/$RUN_NAME/frames/frames_stream/frame_%d.png\" index=0 caps=\"image/png,framerate=1/100,width=640,height=480\" loop=false ! pngdec ! videorate ! autovideosink sync=false"]
#run
